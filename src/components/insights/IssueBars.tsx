...
export function IssueBars({ rows, labels }: IssueBarsProps) {
  ...
  return (
    <div className="space-y-2 rounded-xl border border-pebble-border/28 bg-pebble-overlay/[0.05] p-3">
      {rows.map((row) => (
        <div key={row.type} className="space-y-1.5">
          ...
          <div className="h-2 rounded-full border border-pebble-border/25 bg-pebble-canvas/50">
            <div
