...
def _response(ok: bool, exit_code: int | None, stdout: str, stderr: str, timed_out: bool, duration_ms: int):
    return {
        'ok': ok,
        'exitCode': exit_code,
...
def _run_cpp(run_dir: Path, code: str, stdin: str, deadline: float) -> Tuple[int | None, str, str, bool]:
    ...
    if compile_timed_out or compile_exit != 0:
        return compile_exit, compile_stdout, compile_stderr, compile_timed_out

    run_remaining = _remaining_ms(deadline)
    if run_remaining <= 0:
        return None, compile_stdout, compile_stderr or 'Execution timed out before program run.', True
    ...
    return run_exit, compile_stdout + run_stdout, compile_stderr + run_stderr, run_timed_out


def _run_java(run_dir: Path, code: str, stdin: str, deadline: float) -> Tuple[int | None, str, str, bool]:
...
def handler(event, _context):
    ...
    if language not in SUPPORTED_LANGUAGES:
        ...
        return _response(False, None, '', 'language is required (python, javascript, cpp, java).', False, duration_ms)
    ...
    if len(code) > MAX_CODE_CHARS:
        duration_ms = int((time.perf_counter() - started_at) * 1000)
        return _response(False, None, '', f'code exceeds maximum size of {MAX_CODE_CHARS} characters.', False, duration_ms)
    ...
    try:
        ...
        if language == 'python':
            ...
            exit_code, stdout, stderr, timed_out = _run_cpp(run_dir, code, stdin, deadline)
        else:
            exit_code, stdout, stderr, timed_out = _run_java(run_dir, code, stdin, deadline)
        ...
        return _response(ok, exit_code, stdout, stderr, timed_out, duration_ms)
    except Exception as exc:  # pragma: no cover - defensive fallback
        duration_ms = int((time.perf_counter() - started_at) * 1000)
